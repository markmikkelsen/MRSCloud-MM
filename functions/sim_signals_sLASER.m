...
for ii = 1:length(MRS_opt)
    ...
    switch mega_or_hadam
        case 'UnEdited'
            ...
            delays(2) = taus(2) - (refTp/2 + refTp/2);
            delays(3) = taus(3) - (refTp/2 + refTp/2);
            delays(4) = taus(4) - (refTp/2 + refTp/2);
    ...
    switch mega_or_hadam
        ...
        case 'UnEdited'
            ...
            parfor (X = 1:length(MRS_opt(ii).x), MRS_opt.parallelize.workers)
                ...
                d_temp     = sim_apply_pfilter(d_temp, MRS_opt(ii).H, -1);
                d_temp2{X} = sim_evolve(d_temp, MRS_opt(ii).H, (delays(3)/1e3)); %#ok<*AGROW> 
            end
        ...
        case {'MEGA','HERMES','HERCULES'}
            ...
            if any(strcmp(MRS_opt(ii).seq, {'HERCULES','HERMES'}))
            ...
            parfor (X = 1:length(MRS_opt(ii).x), MRS_opt.parallelize.workers)
                ...
                if ~strcmp(MRS_opt(ii).seq, 'MEGA')
                    ...
                    [outA_temp{X}, outB_temp{X}, outC_temp{X}, outD_temp{X}] = ...
                        sim_mega_slaser_shaped_ultrafast_Ref2(MRS_opt(ii), MRS_opt(ii).Qrefoc{X}, outA_temp{X}, outB_temp{X}, outC_temp{X}, outD_temp{X});
                else
                ...
                switch mega_or_hadam
                    case 'MEGA'
                        ...
                        outA_temp{X}      = sim_evolve(outA_temp{X}, MRS_opt(ii).H, delays(4)/1e3);
                        outB_temp{X}      = sim_apply_pfilter(outB_temp{X},MRS_opt(ii).H, -1);
                        outB_temp{X}      = sim_evolve(outB_temp{X}, MRS_opt(ii).H, delays(4)/1e3);
                    case {'HERMES', 'HERCULES'}
                        outA_temp{X}      = sim_apply_pfilter(outA_temp{X}, MRS_opt(ii).H, -1);
                        outA_temp{X}      = sim_evolve(outA_temp{X}, MRS_opt(ii).H, delays(4)/1e3);
                        outB_temp{X}      = sim_apply_pfilter(outB_temp{X}, MRS_opt(ii).H, -1);
                        ...
                        outC_temp{X}      = sim_evolve(outC_temp{X}, MRS_opt(ii).H, delays(4)/1e3);
                        outD_temp{X}      = sim_apply_pfilter(outD_temp{X}, MRS_opt(ii).H, -1);
                        outD_temp{X}      = sim_evolve(outD_temp{X}, MRS_opt(ii).H, delays(4)/1e3);
                end
    ...
    switch mega_or_hadam
        ...
        case 'UnEdited'
            parfor (Y = 1:length(MRS_opt(ii).y), MRS_opt.parallelize.workers)
                ...
                d_temp     = sim_apply_pfilter(d_temp,MRS_opt(ii).H,-1);                   %Apply p_filter % added on Dec 9
                d_temp2{Y} = sim_evolve(d_temp,MRS_opt(ii).H,(delays(5)/1e3));                     %Evolve by (delays(1)+delays(2))/2
            end
            ...
            parfor (Y = 1:length(MRS_opt(ii).y), MRS_opt.parallelize.workers)
                ...
                switch mega_or_hadam
                    case 'MEGA'
                        ...
                        outA_temp{Y}       = sim_evolve(outA_temp{Y}, MRS_opt(ii).H, delays(5)/1e3);
                        outB_temp{Y}       = sim_apply_pfilter(outB_temp{Y}, MRS_opt(ii).H, +1);
                        outB_temp{Y}       = sim_evolve(outB_temp{Y}, MRS_opt(ii).H, delays(5)/1e3);
                    case {'HERMES', 'HERCULES'}
                        outA_temp{Y}       = sim_apply_pfilter(outA_temp{Y}, MRS_opt(ii).H, +1);
                        outA_temp{Y}       = sim_evolve(outA_temp{Y}, MRS_opt(ii).H, delays(5)/1e3);
                        outB_temp{Y}       = sim_apply_pfilter(outB_temp{Y},MRS_opt(ii).H, +1);
                        ...
                        outC_temp{Y}       = sim_evolve(outC_temp{Y}, MRS_opt(ii).H, delays(5)/1e3);
                        outD_temp{Y}       = sim_apply_pfilter(outD_temp{Y},MRS_opt(ii).H, +1);
                        outD_temp{Y}       = sim_evolve(outD_temp{Y}, MRS_opt(ii).H, delays(5)/1e3);
                end
...
% if not PRESDS
d       = sim_evolve(d_temp,MRS_opt.H,MRS_opt.delays(2)/1e3);                      %Evolve by delays(2)
...
d1           = sim_evolve(d1,MRS_opt.H,MRS_opt.delays(6)/1e3);                            %Evolve by delays(4)
d2           = sim_evolve(d2,MRS_opt.H,MRS_opt.delays(6)/1e3);                            %Evolve by delays(4)
if ~strcmp(MRS_opt.seq, 'MEGA')
    d3       = sim_evolve(d3,MRS_opt.H,MRS_opt.delays(6)/1e3);                            %Evolve by delays(4)
    d4       = sim_evolve(d4,MRS_opt.H,MRS_opt.delays(6)/1e3);                            %Evolve by delays(4)
end
...
d1           = sim_evolve(d1,MRS_opt.H,MRS_opt.delays(7)/1e3);                            %Evolve by delays(5)
d2           = sim_evolve(d2,MRS_opt.H,MRS_opt.delays(7)/1e3);                            %Evolve by delays(5)
if ~strcmp(MRS_opt.seq, 'MEGA')
    d3       = sim_evolve(d3,MRS_opt.H,MRS_opt.delays(7)/1e3);                            %Evolve by delays(5)
    d4       = sim_evolve(d4,MRS_opt.H,MRS_opt.delays(7)/1e3);                            %Evolve by delays(5)
else
    d3       = 0;
    d4       = 0;
end
...
if ~any(strcmp(MRS_opt.seq, {'UnEdited','MEGA'}))
