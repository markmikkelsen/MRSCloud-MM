...
if ~isPhsMod 
    ...
    if ischar(type)
        ...
        elseif strcmp(type,'ref')
            flipCyc=0.5;  %180 degress is 0.5 cycles;
        elseif strcmp(type,'inv')
    ...
    % MM (250905)
    TF=islocalmin(mv(3,:)); % Find first point where Mz is -1
    index=find(TF,1);
    w1max=sc(index);
